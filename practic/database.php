...
$requests = array(
  ...
  '3' => "SELECT ia.id AS 'Код агента', CONCAT(ia.first_name, ' ', ia.last_name, ' ', ia.patronymic) AS 'Ф.И.О. агента',  c.id AS 'Код контракта', c.date AS 'Дата подписания контракта', CONCAT(cl.first_name, ' ', cl.last_name, ' ', cl.patronymic) AS 'Ф.И.О. клиента', c.amount_of_insurance AS 'Сумма страхования', it.name AS 'Тип страхования' FROM insurance_agents ia JOIN contracts c ON ia.id = c.id_insurance_agent JOIN insurance_type it ON it.id = c.id_insurance_type JOIN clients cl ON cl.id = c.id_clients WHERE c.amount_of_insurance >= 200000;",
  '2' => "SELECT ia.id AS 'Код агента', CONCAT(ia.first_name, ' ', ia.last_name, ' ', ia.patronymic) AS 'Ф.И.О. агента', ia.bet AS 'Процент вознаграждения' FROM insurance_agents ia WHERE bet >= 20 AND bet <= 50;",
  '1' => "SELECT cl.id AS 'Код клиента', CONCAT(cl.first_name, ' ', cl.last_name, ' ', cl.patronymic) AS 'Ф.И.О. клиента', cl.discount AS 'Персональная скидка %' FROM clients cl WHERE last_name = 'Иванов';",
  '7' => "SELECT c.id_insurance_agent AS 'Код агента', CONCAT(ia.first_name, ' ', ia.last_name, ' ', ia.patronymic) AS 'Ф.И.О. агента', AVG(c.amount_of_insurance) AS 'Среднее значение суммы страхования' FROM contracts c JOIN insurance_agents ia ON ia.id = c.id_insurance_agent GROUP BY c.id_insurance_agent;",
);
